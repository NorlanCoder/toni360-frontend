    ...
    <header className="absolute top-0 left-0 right-0 z-10 px-6 py-4 bg-white/90 backdrop-blur-sm">
      ...
      <div className="max-w-[98vw] xl:max-w-[1600px] mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <Link href="/client/connexion" className="px-6 py-2 border-2 border-toni-green-dark-2 text-toni-green-dark-2 font-bold rounded-full hover:bg-toni-green-dark-2 hover:text-white transition">
            Se connecter
          </Link>
          {/* Bouton S'inscrire - fond vert (toni-green) */}
          <Link href="/client/inscription" className="px-6 py-2 bg-toni-green-dark-2 text-white rounded-full hover:bg-toni-green-dark transition">
            S&apos;inscrire
          </Link>
        </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-4 px-6 py-4 bg-white/95 rounded-lg backdrop-blur-sm shadow-lg">
          <div className="flex flex-col gap-3">
            <Link href="/client/connexion" className="w-full px-6 py-3 border-2 border-toni-green-dark-2 text-toni-green-dark-2 font-bold rounded-full hover:bg-toni-green-dark-2 hover:text-white transition text-center">
              Se connecter
            </Link>
            <Link href="/client/inscription" className="w-full px-6 py-3 bg-toni-green text-white rounded-full hover:bg-toni-green-dark transition text-center">
              S&apos;inscrire
            </Link>
          </div>
        </div>
