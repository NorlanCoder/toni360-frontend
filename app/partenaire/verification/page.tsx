...
import { getPartnerProfile, resendOtp, verifyOtp } from "@/lib/api/auth";
...
function PartenaireVerificationContent() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem(PARTNER_SESSION_KEY);
    if (!stored) {
  ...
  const handleVerify = async () => {
    ...
    if (!emailReal) { toast.error("Session expirée. Veuillez recommencer."); router.push("/partenaire/connexion"); return; }
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      const response = await resendOtp({ login: emailReal, purpose, actor_type: "user" });
      if (response.success) {
  ...
  return (
    <div className="flex min-h-screen">
      <div className="relative hidden lg:block lg:w-3/5">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/images/imgc.jpg')" }} />
      </div>
      ...
      >
        <div className="w-full max-w-md">
          ...
          <div className="mt-4 text-center text-xs text-gray-400">
            <Link href="/partenaire/connexion" className="hover:underline">← Retour à la connexion</Link>
          </div>
        </div>
      </div>
    </div>
...
export default function PartenaireVerificationPage() {
  return (
    <Suspense fallback={null}>
      <PartenaireVerificationContent />
    </Suspense>
  );
}
